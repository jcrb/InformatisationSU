...
```{r region, echo=FALSE, comment=""}
summary(d$Région)

barplot(summary(d$Région), main="Nombre d'établissements avec service d'urgence", ylab="Nombre", las=3, cex.names=0.7)
text(6,85, "moyenne: 24 établissements/région")
```
...
SU contactés en attente de réponse: `r logiciels[27] + logiciels[26]`
...
```{r, echo=FALSE, comment=""}
sum.log <- rowSums(reg.log) # nb total de logiciels par région
...
leg <- as.character(levels(cut(region@data$rapport, breaks = eq.quart, include.lowest = TRUE, right = FALSE)))

par(mar=c(0,0,1,0))
plot(region, col=eq, border="white")
legend("bottomleft", legend=leg, bty="n", fill=vPal4, cex=0.8, title="Densité")
title(main="Taux de réponses (au 1/09/2014)")
```
